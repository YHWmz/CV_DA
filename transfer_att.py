...
def get_model(args):
    ...
    model = models.TransferforAtt(args.n_class, transfer_loss=args.transfer_loss, base_net=args.backbone, max_iter=args.max_iter,
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, lr_scheduler, args):
    ...
    for e in range(1, args.n_epoch+1):
        ...
        for _ in range(n_batch):
            ...
            clf_loss, transfer_loss, domain_clf_loss = model(data_source, data_target, label_source)
            loss = clf_loss + args.transfer_loss_weight * transfer_loss + 1 * domain_clf_loss
