...
class AdversarialLoss(nn.Module):
    ...
    def forward(self, source, target):
        ...
        source_loss, source_pred = self.get_adversarial_result(source, True, lamb)
        target_loss, target_pred = self.get_adversarial_result(target, False, lamb)
        adv_loss = 0.5 * (source_loss + target_loss)
        return adv_loss, source_pred, target_pred
